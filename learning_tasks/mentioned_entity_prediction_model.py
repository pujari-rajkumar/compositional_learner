...
t1 = datetime.now()
from learning_tasks.authorship_prediction_utils import batchify_data
from learning_tasks.mentioned_entity_prediction_utils import load_data_batch
t2 = datetime.now()
print(t2-t1)
...
t1 = datetime.now()
query_batches = 100
log = open(fpath + 'training_logs/log_r4056_t4056_phase2.txt', 'a+')
if already_done > 0:
    args.pretrained = fpath + 'saved_parameters/link_predictor_phase2_b' + str(already_done - 1) + '.pt'
else:
    state_dict = torch.load(fpath + 'saved_parameters/link_predictor_phase1_b99.pt')
    encoder_state_dict = {}
    for key in state_dict:
        if key.startswith('encoder'):
            encoder_state_dict[key[8:]] = state_dict[key].cpu()
    composer_state_dict = {}
    for key in state_dict:
        if key.startswith('composer'):
            composer_state_dict[key[9:]] = state_dict[key].cpu()
...
if already_done == 0:
    ...
    nn.init.xavier_uniform_(link_predictor.ff1.weight)
    nn.init.xavier_uniform_(link_predictor.ff2.weight)
for query_batch in range(already_done, query_batches):
    ...
    data = load_data_batch(qdb_path, log=log, trim_auth=True)
    t2 = datetime.now()
    ...
    with torch.cuda.device(1):
        ...
        torch.save(link_predictor.state_dict(), fpath + 'saved_parameters/link_predictor_phase2_b' + str(query_batch) + '.pt')
